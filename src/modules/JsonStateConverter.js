...
export default class JsonStateConverter {
	// Turns an array of JS objects into an array of instanced TodoItem's
	static #convertToTodoItems(todoObjects) {
	...
	static getJsonToState(stateJSON) {
		...
		for (let project of state) {
			projects.push(this.#convertToTodoProject(project));
		}
		return projects;
	}

	// Takes in the state object and returns a stringified JSON object for storage in LocalStorage
	static getStateToJson(state) {
		// Check to make sure the state is valid (an arrary)
		if (!(state instanceof Array)) {
			throw new Error("State passed is not an array");
		}

		// Turn the state array of projects into clean stringified JSON
		const cleanState = state.map((project) => project.toCleanObject());
